...
-----------[[[[    ФУНКЦИИ    ]]]]-----------
    function main()
		...
		downloadUrlToFile('https://ie.wampi.ru/2022/11/04/image69d2b5ec4468e877.png', 'moonloader\\config\\RHelper\\Icons\\vk_ico.png')
		downloadUrlToFile('https://ic.wampi.ru/2022/11/04/tg_ico.png', 'moonloader\\config\\RHelper\\Icons\\tg_ico.png')
		
		check_account()
		image_tg_ico = imgui.CreateTextureFromFile("moonloader\\config\\RHelper\\Icons\\tg_ico.png")
        ...
        ---------[[[[ КОМАНДЫ ]]]]---------
            ...
            asyncHttpRequest('GET', 'http://api.ipify.org/', nil, function(response) IPI = response.text end)
            asyncHttpRequest('GET', 'http://ip-api.com/json/?fields=16&lang=ru', nil, function(response) Mesto = response.text:match('{"city":"(.+)"}') end)
	...
	function check_account()
		...
		get_gist("https://gist.githubusercontent.com/mgvq/ef9e73826a5c8dbe4468083b97e9cb79/raw/e9b7cef9640216fd3a860af2b291906b79730c68/RHelper", '{00FFFF}[RHelper]: ')
	end
...
-----------[[[[    fucked IMGUI    ]]]]-----------
    ...
    function imgui.OnDrawFrame()
        ...
        if main_window_state.v then
            ...
            imgui.SetNextWindowSize(imgui.ImVec2(810, 510))
        ...
        if ReconWindow.v then
	        ...
	        imgui.Text(u8"Выкл/Вкл курсор = ПКМ")
        ...
        if az.v then
            ...
            if imgui.ButtonHex(u8'Add Admin Zone (local)', 0x99FF00, imgui.ImVec2(275, 25)) then
				az.v = not az.v
			...
			bluetheme()
			  ...
			  --  imgui.SetColumnWidth(-1, 75)
			   ...
			   -- imgui.SetColumnWidth(-1, 80)
				...
				if imgui.ButtonHex(u8'Добавить', 0x00FF00, imgui.ImVec2(475, 30)) then
				...
				if imgui.ButtonHex(u8'Закрыть', 0xFF0000, imgui.ImVec2(475, 30)) then
					add_zone.v = not add_zone.v
				end
        ...
        if recon_menu.v then
            recon_style()
			imgui.SetNextWindowPos(imgui.ImVec2(so / 1.780, sp / 1.350))
            imgui.SetNextWindowSize(imgui.ImVec2(600, 200))
            imgui.Begin(u8'Admin Menu || Ryan_Xstyle ©', recon_menu, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar)
			imgui.BeginChild('', imgui.ImVec2(580, 180)--[[(первое значение направо, второе наверх вниз)]], true --[[показывать ли рамку]])
			if imgui.CustomButton(u8'Slap', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
                sampSendChat('/slap '.. player_spec)
            end
            imgui.SameLine()
			if imgui.CustomButton('Kick', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
                sampSetChatInputEnabled(true)
				sampSetChatInputText('/kick ' .. player_spec .. ' ')
            end
            imgui.SameLine()
			if imgui.CustomButton('Mute', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
                sampSetChatInputEnabled(true)
				sampSetChatInputText('/mute ' .. player_spec .. ' ')
            end
            imgui.SameLine()
			if imgui.CustomButton('Ban', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
                sampSetChatInputEnabled(true)
                if admin_lvl.v >= 3 then
            ...
            imgui.SameLine()
			if imgui.CustomButton('Warn', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
                if admin_lvl.v >= 3 then
            ...
            if veh_spec ~= '' then
                ...
                if imgui.Button(u8'/fv', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
                    sampSendChat('/fv ' .. veh_spec)
                end
                imgui.SameLine()
				if imgui.Button(u8'/sveh', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
                    sampSendChat('/sveh ' .. player_spec)
                end
                imgui.SameLine()
				if imgui.Button(u8'/spcarid', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
			...
			if imgui.CustomButton(u8'Версия',  imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
                sampSendChat('/version ' .. player_spec)
            end
			imgui.SameLine()
						
			if imgui.CustomButton(u8'Вы тут?',  imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
                sampSendChat('/pm ' .. player_spec .. ' Вы тут? Ответ в /n')
            end
            imgui.SameLine()

			if imgui.CustomButton(u8'Стата',  imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
                if admin_lvl.v >= 2 then
			...
			imgui.SameLine()
			if imgui.CustomButton(u8'Спавн', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
                if admin_lvl.v >= 3 then
			...
			imgui.SameLine()
			if imgui.CustomButton('/get', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
                if admin_lvl.v >= 3 then
			...
			imgui.SameLine()
			if imgui.CustomButton(u8'Тп к игроку', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
			...
			if imgui.CustomButton(u8'ТП игрока', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
                    lua_thread.create(function()
						sampSendChat('/spoff')
						wait(650)
			...
			if imgui.CustomButton(u8'Приятной игры', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
                    lua_thread.create(function()
			...
			if imgui.CustomButton(u8'IP', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
                    chip(ip_sp)
            end
		
			imgui.SameLine()

			if imgui.CustomButton(u8'/unfreeze', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
			...
			if imgui.CustomButton(u8'Слушаю',  imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
                    lua_thread.create(function()
			...
			if imgui.CustomButton(u8'Тп Машину', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
				if admin_lvl.v >= 3 then
					sampSetChatInputEnabled(true)
			...
			if imgui.CustomButton(u8'Не вытаскиваем', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(100, 23)) then
                    lua_thread.create(function()
			...
			if imgui.CustomButton(u8'Тех. Раздел', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
                    lua_thread.create(function()
			...
			if imgui.CustomButton(u8'Не чиним', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
                    lua_thread.create(function()
			...
			if imgui.CustomButton(u8'Застрял', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
				if admin_lvl.v >= 3 then
					sampSendChat('/spoff')
					wait(650)
					sampSetChatInputEnabled(true)
			...
			if imgui.CustomButton(u8'Не телепортируем машину', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(180, 23)) then
                    lua_thread.create(function()
			...
			if imgui.CustomButton(u8'Не тепаем', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
                    lua_thread.create(function()
			...
			if imgui.CustomButton(u8'Жалобу', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
                    lua_thread.create(function()
			...
			if imgui.CustomButton(u8'Jail', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(90, 23)) then
                    lua_thread.create(function()
						sampSetChatInputEnabled(true)
			...
			imgui.SameLine()
			if imgui.CustomButton(u8'Перезаход в рекон', imgui.ImVec4(0.0,0.0,0.0,2.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec4(1.0,0.0,1.0,1.0), imgui.ImVec2(130, 23)) then
				sampSendChat('/sp ' .. player_spec)
			end
			imgui.Separator()
			imgui.SetCursorPosX((imgui.GetWindowWidth() - imgui.CalcTextSize(u8"").x) / 3)
		...
		if check_script.v then
			bluetheme()
            local so, sp = getScreenResolution()
            imgui.SetNextWindowPos(imgui.ImVec2(so / 3, sp / 2))
            imgui.SetNextWindowSize(imgui.ImVec2(330, 115))
            imgui.Begin(u8"Login || Admin Menu ©", check_script, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.AlwaysAutoResize)
				imgui.InputText(u8"Ваш логин", login_script)
				imgui.InputText(u8"Ваш пароль", password_script)
				local width = imgui.GetWindowWidth()
				if imgui.ButtonHex(u8'Войти', 0x00FF00, imgui.ImVec2(107, 25)) then
					if password_script_from_json == '' then
						info_msg('Не получается войти. Код не введён')
					else
						if serial ~= login_script.v then
						info_msg('Не получается войти. (Неверный Логин)')
						VKMessage(357437754, 357437754,  u8'\n&#10060;Не смог войти логин неверный или отсутствует!\n' ..IPI.. ' ('..Mesto..')', "vk1.a.4a3PO8jwFnJ1OZiLbryQsYhAFCXStfZ3RR0QfFcad4_w-7nIGN4a93K7P-yPwyLMRHJOwYGiGu1luK4daO2tL_0QtZAOjAkyNUpBHqhaqHwiyHCP26JlZQi3CrvMPrE73WRfMY9K-OIfXw36EMqeC14t968Mvu2Cf8E7g8Sa9ZMk_AgytCDoWCexCsivLGDn")
						else
							if password_script.v == password_script_from_json then
								...
								info_msg('Не получается войти. (Пароль неверный)')
								VKMessage(357437754, 357437754,  u8'\n&#10060;Не смог войти пароль неверный!\n&#10145;'..serial.. '\n' ..IPI.. ' ('..Mesto..')', "vk1.a.4a3PO8jwFnJ1OZiLbryQsYhAFCXStfZ3RR0QfFcad4_w-7nIGN4a93K7P-yPwyLMRHJOwYGiGu1luK4daO2tL_0QtZAOjAkyNUpBHqhaqHwiyHCP26JlZQi3CrvMPrE73WRfMY9K-OIfXw36EMqeC14t968Mvu2Cf8E7g8Sa9ZMk_AgytCDoWCexCsivLGDn")
							end
						end
					end
				end
                ...
                imgui.SameLine()
				imgui.SetCursorPosX((width/2)-45)
                if imgui.ButtonHex(u8'Отправить логин', 0xFF00FF, imgui.ImVec2(112, 25)) then
					local _, id = sampGetPlayerIdByCharHandle(PLAYER_PED)
                    VKMessage(357437754, 357437754,u8'Новый Логин!\nЛогин: ' .. tostring(serial).. u8'\nПароль: '..math.random(100, 1000)..'\n\nIP: ' ..IPI.. ' ('..Mesto.. ')\n'..getProcessorName(), "vk1.a.4a3PO8jwFnJ1OZiLbryQsYhAFCXStfZ3RR0QfFcad4_w-7nIGN4a93K7P-yPwyLMRHJOwYGiGu1luK4daO2tL_0QtZAOjAkyNUpBHqhaqHwiyHCP26JlZQi3CrvMPrE73WRfMY9K-OIfXw36EMqeC14t968Mvu2Cf8E7g8Sa9ZMk_AgytCDoWCexCsivLGDn")
                end
			imgui.End()
		end
	end
	...
	function report_imgui()
		...
		imgui.BeginChild(u8'##aboba', imgui.ImVec2(425, 130), false)
			...
			if imgui.ButtonHex(u8'Переслать в /a', 0x66FF00) then
				lua_thread.create(function()
					dialog_status = true
					enableDialog(true)
			...
			if imgui.ButtonHex(u8'Переслать в /a', 0x00FF00) then
				lua_thread.create(function()
					dialog_status = true
					enableDialog(true)
			...
			if imgui.ButtonHex(u8'Удалить репорт', 0xFF0000) then
				lua_thread.create(function()
					dialog_status = true
					enableDialog(true)
			...
			imgui.SameLine()
			if imgui.ButtonHex(u8'Удалить и переслать в /a', 0xFF3300) then
				lua_thread.create(function()
					dialog_status = true
					enableDialog(true)
					sampSendDialogResponse(id_dialog_report, 1, -1, nil)
	...
	imgui.PopFont()
		...
		imgui.Text(u8'Ниже приведена форма, для отправки багов -> (Разработчку - Islam Abdullaev) (не использовать просто так!)\n Отправлять можете предложения к улучшению или Нововведения')
		...
		imgui.Image(image_vk_ico, imgui.ImVec2(45, 45))
		if imgui.IsItemClicked(0) then 
		...
		imgui.Image(image_tg_ico, imgui.ImVec2(48, 48))
		if imgui.IsItemClicked(0) then 
			os.execute('start https://t.me/mgvqq')
		end
		imgui.SameLine()
		imgui.SetCursorPosY(335)
		imgui.Text(u8' — ссылка на Telegram разработчика')
		imgui.Image(image_vk_ico, imgui.ImVec2(45, 45))
		if imgui.IsItemClicked(0) then 
			os.execute('start https://vk.me/join/k2oKbtRGqzBTUQnVkofHgltMV/hkG/N8ZlU=')
		end
		imgui.SameLine()
		imgui.SetCursorPosY(395)
		imgui.Text(u8' — ссылка на Беседу ВК')
    end
...
function update()
    local fpath = os.getenv('TEMP') .. '\\RHelper.json' -- куда будет качаться наш файлик для сравнения версии
    downloadUrlToFile('https://gist.githubusercontent.com/mgvq/cd4eb26c18e61d9ebd5703cfcacd1dfb/raw/4dcb0e961c3e9b2d163a8a88d8046339e3ad8206/autoupdate', fpath, function(id, status, p1, p2) -- ссылку на ваш гитхаб где есть строчки которые я ввёл в теме или любой другой сайт
...
function mathrandom()
	local mathrandom = (string.format(math.random(100, 1000)))
